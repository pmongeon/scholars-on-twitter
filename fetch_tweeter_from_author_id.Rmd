...
This code uses the APIs of OpenAlex, Crossref, and Twitter to fetch the potential Twitter accounts of an OpenAlex author. The only requirement to run this code is a bearer token for the Twitter API
...
```{r echo=FALSE}
auth_setup_default()
keys <- read.csv("c:/keys/twitter_keys.csv")
appname <- "Données de recherche" # PUT THE NAME OF YOUR APP HERE
bearer <- keys %>% filter(name == "Bearer token") %>%
  select(key) %>%
  as.character()
```
